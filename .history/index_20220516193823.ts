...
const filterCourses = (range: [price, price], courses: course[]): course[] => {
	...
	const suitableСourses: course[] = courses.filter((course: course) => {
		if (from === null) {
			if ((course.prices !== null && course.prices[0] <= to) && course.prices[1] === null || to === null || course.prices[1] <= to) {
				return true;
			}
		}
		if (to === null) {
			if (course.prices[0] === null || from === null || course.prices[0] >= from) {
				return true;
			}
		}
	});

	return suitableСourses;
};
